...
from app.services.ai_manager import get_ai_manager, ConversationMessage
...
async def test_openrouter_integration():
    ...
    try:
        ...
        logger.info(f"🔄 Fallback тест - использован провайдер: {simple_response.provider}")
        ...
        for provider, provider_stats in stats["provider_stats"].items():
            logger.info(f"   {provider}: {provider_stats['successes']}/{provider_stats['requests']} успешно")
