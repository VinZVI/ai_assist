...
from app.database import init_db, check_connection, close_db
...
async def test_database_connection() -> None:
    ...
    try:
        ...
        logger.info(f"📊 DATABASE_URL: {config.database.database_url.split('@')[1] if '@' in config.database.database_url else 'скрыт'}")
...
async def test_models_import() -> None:
    ...
    try:
        from app.models import User, Conversation, Base
        from app.models.user import UserCreate, UserResponse
        from app.models.conversation import ConversationCreate, MessageRole
        
        logger.info("✅ Все модели успешно импортированы")
        logger.info(f"📋 Доступные модели: User, Conversation")
        logger.info(f"🔧 Базовый класс: {Base}")
        logger.info(f"📝 Pydantic схемы: UserCreate, UserResponse, ConversationCreate")
        logger.info(f"🏷️ Enums: MessageRole")
