...
down_revision: str | Sequence[str] | None = "add_payments_table"
...
def upgrade() -> None:
    ...
    # Check if assistant_personalities table exists
    conn = op.get_bind()
    ...
    op.alter_column(
        ...
        existing_server_default=sa.text(
            "nextval('assistant_personalities_id_seq'::regclass)"
        ),
    )
    op.alter_column(
        "assistant_personalities",
    ...
    op.drop_constraint(
        op.f("assistant_personalities_name_key"),
        "assistant_personalities",
        type_="unique",
    )
    op.create_index(
        op.f("ix_assistant_personalities_name"),
        "assistant_personalities",
        ["name"],
        unique=True,
    )
    ...
    op.create_index(
        op.f("ix_conversations_personality_id"),
        "conversations",
        ["personality_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_assistant_personalities_name"), table_name="assistant_personalities"
    )
