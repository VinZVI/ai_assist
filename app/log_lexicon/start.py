...
START_COMMAND_PROCESSED = "✅ Успешно обработана команда /start для пользователя {user_id}"
START_COMMAND_ERROR = "💥 Ошибка в обработчике /start для пользователя {user_id}: {error}"
...
START_USER_CREATION_ERROR = "❌ Ошибка целостности при создании пользователя {user_id}: {error}"
START_UNEXPECTED_ERROR = "💥 Неожиданная ошибка при работе с пользователем {user_id}: {error}"
