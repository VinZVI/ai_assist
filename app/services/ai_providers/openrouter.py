...
class OpenRouterProvider(BaseAIProvider):
    ...
    async def _make_api_request(
        ...
        for attempt in range(self._max_retries):
            try:
                logger.debug(f"🚀 Отправка запроса к OpenRouter API (попытка {attempt + 1})")
                ...
                if response.status_code == 200:
                    logger.info(f"✅ Успешный ответ от OpenRouter API за {response_time:.2f}с")
                ...
                if response.status_code == 429:
                    if attempt < self._max_retries - 1:
                        ...
                        logger.warning(f"⏳ Rate limit достигнут в OpenRouter. Ожидание {delay}с...")
                ...
                if response.status_code >= 500:
                    if attempt < self._max_retries - 1:
                        ...
                        logger.warning(f"🔄 Ошибка сервера OpenRouter {response.status_code}. Повтор через {delay}с...")
                        await asyncio.sleep(delay)
                        continue
                ...
                if attempt < self._max_retries - 1:
                    delay = self._retry_delay * (attempt + 1)
                    logger.warning(f"⏰ Timeout запроса к OpenRouter. Повтор через {delay}с...")
                    await asyncio.sleep(delay)
                    continue
                raise APIConnectionError(
                ...
                if attempt < self._max_retries - 1:
                    delay = self._retry_delay * (attempt + 1)
                    logger.warning(f"🌐 Ошибка подключения к OpenRouter. Повтор через {delay}с...")
                    await asyncio.sleep(delay)
                    continue
                raise APIConnectionError(
