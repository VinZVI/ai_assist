            ...
            try:
                ...
                if response.status_code == 429:
                    if attempt < self._max_retries - 1:
                        ...
                        logger.warning(
                            f"⏳ Rate limit достигнут в DeepSeek. Ожидание {delay}с...",
                        )
                ...
                if attempt < self._max_retries - 1:
                    delay = self._retry_delay * (attempt + 1)
                    logger.warning(
                        f"⏰ Timeout запроса к DeepSeek. Повтор через {delay}с...",
                    )
                    await asyncio.sleep(delay)
                    continue
                raise APIConnectionError(
                ...
                if attempt < self._max_retries - 1:
                    delay = self._retry_delay * (attempt + 1)
                    logger.warning(
                        f"🌐 Ошибка подключения к DeepSeek. Повтор через {delay}с...",
                    )
                    await asyncio.sleep(delay)
                    continue
                raise APIConnectionError(
