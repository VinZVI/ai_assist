...
class AIManager:
    ...
    async def generate_response(
        ...
        if prefer_provider and prefer_provider in self._providers:
            ...
            if (
                self.provider_config.enable_fallback
                and self.provider_config.fallback_provider
                != self.provider_config.primary_provider
            ):
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        healthy_providers = sum(
            1 for p in results["providers"].values() if p.get("status") == "healthy"
        )
