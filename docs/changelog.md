...
## [2025-11-21] - Этап 1.1: Упрощенная система согласия и возрастной верификации
### Добавлено
- Реализована упрощенная система согласия и возрастной верификации для юридического соответствия
...
- Создан пакет compliance с компонентами системы верификации:
  - ConsentManager для управления процессом получения согласий
  - AgeVerificationService для проверки возраста пользователя
  - LegalTexts с юридическими текстами и сообщениями
- Реализован OnboardingHandler для управления процессом регистрации пользователей
- Созданы OnboardingKeyboards с интерактивными клавиатурами для процесса регистрации
- Добавлен VerificationMiddleware для проверки статуса верификации пользователя
- Расширена функциональность AuthMiddleware для проверки актуальности версий соглашений
- Создана миграция базы данных для добавления новых полей в таблицу пользователей
- Добавлены unit тесты для всех компонентов системы согласия
- Создана подробная документация по системе соответствия (docs/compliance-system.md)
- Добавлена информация об Этапе 1.1 в основную документацию проекта

### Изменено
- Обновлен порядок выполнения middleware - VerificationMiddleware добавлен после EmotionalProfilingMiddleware
- Улучшена обработка текстовых сообщений в VerificationMiddleware для разрешения общения без полной верификации
- Расширена функциональность AuthMiddleware для автоматической проверки актуальности версий соглашений
- Обновлены обработчики сообщений для корректной работы с неверифицированными пользователями

### Исправлено
- Исправлена проблема с блокировкой текстовых сообщений для неверифицированных пользователей
- Улучшена обработка callback запросов в VerificationMiddleware
- Исправлена ошибка с автоматическим истечением срока действия верификации при обновлении соглашений

## [2025-10-18] - Фаза 2: Устранение циркулярных импортов и оптимизация производительности
### Добавлено
- Реализована система dependency injection для устранения циркулярных импортов
- Созданы новые файлы: app/core/dependencies.py, app/core/service_registry.py, app/services/conversation_service_new.py
- Добавлены утилиты для оптимизации производительности БД (app/utils/db_optimization.py)
- Реализован пакетный процессор для операций с БД (app/services/batch_processor.py)
- Создана система мониторинга производительности (app/services/performance_monitor.py)
- Добавлены метрики и алерты для отслеживания состояния системы
- Создан веб-дашборд для визуализации метрик производительности
- Добавлены фоновые задачи для управления батч-процессором и очисткой кэша
- Реализована новая структура контекста диалога с разделением на последние 5 сообщений пользователя и 5 ответов ИИ
- Создан класс UserAIConversationContext для управления контекстом диалога
- Добавлена документация по структуре контекста диалога (docs/conversation-context.md)

### Изменено
- Рефакторинг conversation_service для устранения циркулярных импортов
- Обновлены middleware компоненты для использования dependency injection
- Модифицированы основные сервисы для работы с новой архитектурой
- Улучшена инициализация приложения через централизованный service registry
- Обновлена структура контекста диалога для хранения отдельно пользовательских сообщений и ответов ИИ
- Улучшена документация по архитектуре кэширования с информацией о ConversationPersistence

### Исправлено
- Устранены все циркулярные импорты в проекте
- Оптимизированы запросы к базе данных с помощью дополнительных индексов
- Уменьшено количество запросов к БД за счет пакетной обработки операций
- Исправлена структура контекста диалога для правильного хранения истории
