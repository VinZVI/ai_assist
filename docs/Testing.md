...
**Общее покрытие кода:** 51% (854/1752 строк не покрыты)
...
|-----------|-------------------|---------|
| **Всего собрано** | 112 тестов | ✅ |
| **Прошли успешно** | 45 тестов | ✅ |
| **Провалились** | 46 тестов | ❌ |
| **Пропущены** | 5 тестов | ⏭️ |
| **Ошибки** | 16 тестов | 💥 |
...
- **Всего тестов**: 10
- **Статус**: 🔴 Много проблем
- **Основные проблемы**:
  - Отсутствует .env.example файл
  - Проблемы с валидацией Pydantic
...
- **Статус**: 🟡 Частично работает  
- **Основные проблемы**:
  - Проблемы с async context managers
  - Некорректные моки для SQLAlchemy
  - Экспорты модулей не соответствуют реальности
...
### Этап 2: Создание недостающих тестов (Приоритет 2)
...
*Документация обновлена: 2025-09-20*  
*Версия: 1.0.0*  
*Автор: AI Assistant*