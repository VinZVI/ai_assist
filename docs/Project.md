...
12. **VerificationMiddleware** - проверка верификации пользователя
...
9. VerificationMiddleware (проверка верификации пользователя)
10. ConversationMiddleware (для сохранения диалогов)
11. MessageCountingMiddleware (для подсчета сообщений и ограничения по лимиту)
12. MetricsMiddleware (последним для сбора полной информации)

### 2.4 Этап 1.1: Упрощенная система согласия и возрастной верификации

Для обеспечения юридического соответствия и защиты от несовершеннолетних пользователей реализована упрощенная система согласия и возрастной верификации.

**Компоненты системы:**

1. **Модель пользователя (User)** - расширена новыми полями для отслеживания состояния верификации:
...
2. **ConsentManager** - управляет процессом получения согласий пользователя
3. **AgeVerificationService** - обрабатывает проверку возраста пользователя
4. **OnboardingHandler** - управляет процессом регистрации пользователя
5. **OnboardingKeyboards** - предоставляет интерактивные клавиатуры для процесса регистрации
6. **VerificationMiddleware** - проверяет статус верификации пользователя перед обработкой запросов
...
### 2.5 Система Dependency Injection
...
### 2.6 Структура контекста диалога

Для оптимизации взаимодействия с пользователями реализована новая структура контекста диалога, которая хранит отдельно последние 5 сообщений пользователя и 5 ответов ИИ. Подробнее см. в [документации по контексту диалога](conversation-context.md).

**Компоненты системы контекста диалога:**

1. **UserAIConversationContext** - основная структура контекста диалога
   - Автоматическое ограничение количества сообщений до 5 для каждого типа
   - Разделение на пользовательские сообщения и ответы ИИ
   - Методы для добавления сообщений и получения объединенной истории

2. **ConversationPersistence** - система персистентности контекстов диалогов
   - Тройная защита сохранения (память, Redis, база данных)
   - Фоновый процесс периодического бэкапа
   - Восстановление контекстов при перезапуске приложения

3. **ConversationService** - сервис для работы с диалогами
   - Управление контекстом диалогов пользователей
   - Сохранение контекста в кэше и базе данных
   - Получение истории последних сообщений пользователя

**Преимущества новой структуры:**
- Оптимизация памяти за счет ограничения количества сообщений
- Улучшенное понимание контекста благодаря разделению на типы сообщений
- Повышенная производительность за счет эффективного кэширования

### 2.7 Система батчинга операций
...
### 2.8 Система мониторинга и аналитики
...
### 2.10 Система масштабирования и высокой доступности
...
│   ├── conversation-context.md # Структура контекста диалога
