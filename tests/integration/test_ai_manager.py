...
class TestAIManagerFallbackLogic:
    ...
    @pytest.mark.asyncio
    async def test_successful_primary_provider(self, manager_with_mocked_providers, mock_conversation_messages, mock_ai_response):
    ...
    @pytest.mark.asyncio
    async def test_fallback_on_primary_failure(self, manager_with_mocked_providers, mock_conversation_messages, mock_ai_response):
        """Тест fallback при сбое основного провайдера."""
        manager, mock_openrouter, mock_deepseek = manager_with_mocked_providers
        
        # OpenRouter падает, DeepSeek работает
        mock_openrouter.generate_response.side_effect = APIConnectionError("OpenRouter недоступен", "openrouter")
    ...
    @pytest.mark.asyncio
    async def test_both_providers_fail(self, manager_with_mocked_providers, mock_conversation_messages):
        """Тест когда оба провайдера недоступны."""
        manager, mock_openrouter, mock_deepseek = manager_with_mocked_providers
        
        # Оба провайдера падают
        mock_openrouter.generate_response.side_effect = APIConnectionError("OpenRouter недоступен", "openrouter")
        mock_deepseek.generate_response.side_effect = APIConnectionError("DeepSeek недоступен", "deepseek")
        
        with pytest.raises(APIConnectionError, match="DeepSeek недоступен"):
            await manager.generate_response(mock_conversation_messages)
    
    @pytest.mark.asyncio
    async def test_rate_limit_fallback(self, manager_with_mocked_providers, mock_conversation_messages, mock_ai_response):
        """Тест fallback при превышении лимита запросов."""
        manager, mock_openrouter, mock_deepseek = manager_with_mocked_providers
        
        # OpenRouter выдает rate limit, DeepSeek работает
        mock_openrouter.generate_response.side_effect = APIRateLimitError("Rate limit exceeded", "openrouter")
        mock_deepseek.generate_response.return_value = mock_ai_response
...
class TestAIManagerCaching:
    ...
    @pytest.mark.asyncio
    async def test_cache_hit(self, manager_with_mocked_providers, mock_conversation_messages):
        ...
        with patch.object(manager, 'generate_response') as mock_generate:
            ...
            # Первый запрос
            response1 = await manager.generate_response(mock_conversation_messages, use_cache=True)
            # Второй запрос (тот же контент)
            response2 = await manager.generate_response(mock_conversation_messages, use_cache=True)
            
            assert response2.cached is True
            assert response2.response_time < response1.response_time
    
    @pytest.mark.asyncio 
    async def test_cache_disabled(self, manager_with_mocked_providers, mock_conversation_messages, mock_ai_response):
        ...
        # Запрос без кеша
        response = await manager.generate_response(mock_conversation_messages, use_cache=False)
...
class TestAIManagerErrorHandling:
    ...
    async def test_invalid_messages_format(self, mock_ai_manager):
        ...
        mock_ai_manager.generate_response.side_effect = ValueError("Сообщения должны быть списком")
    ...
    async def test_empty_messages_list(self, mock_ai_manager):
        ...
        # Мок должен выбрасывать ошибку
        mock_ai_manager.generate_response.side_effect = ValueError("Список сообщений не может быть пустым")
        
        with pytest.raises(ValueError, match="Список сообщений не может быть пустым"):
            await mock_ai_manager.generate_response([])
    
    @pytest.mark.asyncio
    async def test_authentication_error_handling(self, manager_with_mocked_providers, mock_conversation_messages):
        """Тест обработки ошибок аутентификации."""
        manager, mock_openrouter, mock_deepseek = manager_with_mocked_providers
        
        # Оба провайдера выдают ошибку аутентификации
        mock_openrouter.generate_response.side_effect = APIAuthenticationError("Invalid API key", "openrouter")
        mock_deepseek.generate_response.side_effect = APIAuthenticationError("Invalid API key", "deepseek")
        
        with pytest.raises(AIProviderError, match="Ошибки аутентификации во всех провайдерах"):
            await manager.generate_response(mock_conversation_messages)
    
    @pytest.mark.asyncio
    async def test_mixed_error_handling(self, manager_with_mocked_providers, mock_conversation_messages, mock_ai_response):
