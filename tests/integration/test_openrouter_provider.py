...
class TestOpenRouterProviderInitialization:
    ...
    def test_provider_configuration_check(self, mock_config: AppConfig) -> None:
        ...
        with patch(
            "app.services.ai_providers.openrouter.get_config",
        ) as mock_get_config:
...
class TestOpenRouterErrorHandling:
    ...
    async def test_authentication_error(
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_response):
            with pytest.raises(
                APIAuthenticationError,
                match="Неверный API ключ OpenRouter",
            ):
                await provider.generate_response(sample_messages)
    ...
    async def test_quota_exceeded_error(
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_response):
            with pytest.raises(
                APIQuotaExceededError,
                match="Недостаточно средств на счете OpenRouter API",
            ):
                await provider.generate_response(sample_messages)
    ...
    async def test_rate_limit_error(
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_response):
            with pytest.raises(
                APIRateLimitError,
                match="Превышен лимит запросов к OpenRouter API",
            ):
                await provider.generate_response(sample_messages)
    ...
    async def test_server_error(
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_response):
            with pytest.raises(
                APIConnectionError,
                match="Ошибка сервера OpenRouter: 500",
            ):
                await provider.generate_response(sample_messages)
...
class TestOpenRouterResponseParsing:
    ...
    async def test_empty_content_handling(self, provider: OpenRouterProvider) -> None:
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_response):
            with pytest.raises(
                APIConnectionError,
                match="Пустой ответ от OpenRouter API",
            ):
    ...
    async def test_missing_choices_handling(self, provider: OpenRouterProvider) -> None:
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_response):
            with pytest.raises(
                APIConnectionError,
                match="Некорректный формат ответа от OpenRouter API",
            ):
                await provider.generate_response(messages)
