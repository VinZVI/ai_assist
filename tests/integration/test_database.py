...
class TestDatabaseInitialization:
    ...
    @patch("app.database.get_config")
    @patch("app.database.create_engine")
    async def test_init_db_failure(self, mock_create_engine, mock_get_config):
...
class TestDatabaseTables:
    ...
    async def test_create_tables_success(self, mock_get_engine):
        ...
        # Мокируем асинхронный контекстный менеджер правильно
    ...
    async def test_create_tables_failure(self, mock_get_engine):
        ...
        # Мокируем асинхронный контекстный менеджер с исключением
        mock_context_manager = AsyncMock()
    ...
    async def test_drop_tables_production_mode(self, mock_get_config):
        ...
        with pytest.raises(
            RuntimeError,
            match="Удаление таблиц разрешено только в debug режиме",
        ):
            await drop_tables()
